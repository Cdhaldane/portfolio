...
// Neon–Vercel integration). Runs on the Node runtime; uses `pg`. If no DB is
// configured it degrades gracefully rather than erroring.
...
// No-op in production (Vercel provides it there).
    ...
    if (req.method === "POST") {
      ...
      const recent = await p.query(
        `SELECT 1 FROM guestbook
          WHERE ip_hash = $1 AND created_at > now() - interval '30 seconds'
          LIMIT 1`,
        [ipHash]
      );
      if (recent.rowCount) {
